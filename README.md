...
Rework is a Progressive Web App, so once deployed (over HTTPS) you can install
it to your phone or desktop and it works offline:

- **iOS Safari**: Share → "Add to Home Screen".
- **Android Chrome**: menu → "Install app" (or the install prompt).
- **Desktop Chrome/Edge**: the install icon in the address bar.

After the first visit, a service worker (`public/sw.js`) caches the app shell
so it opens with no network. Your data is already local, so the whole app is
usable offline — only live food search, barcode lookups, and photo scanning
need a connection (they fail soft to your local library when offline). App
icons are generated by `scripts/gen_icons.py` (no dependencies) if you ever
want to regenerate or recolor them.

Nothing here is required for local dev: the service worker only registers in a
production build, so `npm run dev` and its hot-reload are never intercepted.
